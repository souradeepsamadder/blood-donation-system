...
$status = $_GET['c'];
$sql = "update hospital set hospital.Status='approval' where Slno='$status'";
mysqli_query($conn, $sql);
echo "<script>alert('approve the hospital')</script>";
$result = mysqli_query($conn, "select * from hospital where Slno='$status'");
...
<header>
        <center>
            ...
            <p> We Blood Bucket team wants to thank you for taking out valuable time from your busy schedule and
                becoming priceless part in the list of Recipient.</p><br>
...
echo "<script>window.location.href='approval_hospital.php'</script>";
