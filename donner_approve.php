...
$status = $_GET['c'];
$sql = "update donner set donner.Status='approve' where Sno='$status'";
mysqli_query($conn, $sql);
echo "<script>alert('approve the donner')</script>";
$result = mysqli_query($conn, "select * from donner where Sno='$status'");
...
<header>
        <center>
            ...
            <p> We Blood Bucket team wants to thank you for taking out valuable time from your busy schedule and
                becoming priceless part in the list of blood donors.</p><br>
...
echo "<script>window.location.href='approve_donner.php'</script>";
